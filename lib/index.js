...
export default (env = { policies: [], controllers: [] }) => {
  debug('init', env)
  ...
  app.router.get('/swagger.json', async ctx => {
    ctx.body = await fs.readFile(path.join(__dirname, '/swagger.json'), 'utf8')
  })

  app.router.get('/healthcheck', async ctx => {
    ctx.body = 'healthy'
  })
  ...
  // TODO: control flow for bootstrap().then(run) [or run includes bootstrap]
  // env.bootstrap(app)
